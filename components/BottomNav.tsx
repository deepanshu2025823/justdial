...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 shadow-[0_-10px_20px_rgba(0,0,0,0.05)] z-[100] md:hidden">
      <div className="flex justify-between items-center h-[70px] max-w-md mx-auto px-4 relative">
        
        <NavItem icon={<Home size={24} />} label="Home" active />

        <NavItem icon={<Briefcase size={24} />} label="B2B" />
        ...
        <div className="relative -top-5 flex flex-col items-center group">
          ...
          <span className="text-[10px] font-bold text-gray-700 mt-1">Chat</span>
        </div>

        <div className="relative">
          <NavItem icon={<Mail size={24} />} label="Leads" />
          <span className="absolute top-0 right-3 h-2.5 w-2.5 bg-red-600 rounded-full border-2 border-white animate-pulse"></span>
        </div>
...
function NavItem({ icon, label, active = false }: NavItemProps) {
  return (
    <div className="flex flex-col items-center justify-center min-w-[60px] cursor-pointer transition-all active:scale-95 group">
      ...
      <span className={`text-[10px] font-bold mt-1 ${active ? "text-[#0073c1]" : "text-gray-500"}`}>
        {label}
      </span>
    </div>
  );
